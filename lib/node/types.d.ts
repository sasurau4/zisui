...
export declare type ExposedWindow = typeof window & {
    __STORYBOOK_CLIENT_API__: API;
    stories?: StoryKind[];
    emitCatpture(opt: ScreenShotOptions): void;
    waitFor?: () => Promise<any>;
    requestIdleCallback(cb: Function, opt?: {
        timeout: number;
    }): void;
    getCurrentStory: (url: string) => Promise<Story | undefined>;
    optionStore?: {
        [kind: string]: {
            [story: string]: (Partial<ScreenShotOptions>)[];
        };
    };
};
export declare type ZisuiRunMode = "simple" | "managed";
