...
function createOptions() {
    const setting = yargs_1.default
        ...
        .option("serverTimeout", { number: true, default: 20000, description: "Timeout [msec] for starting Storybook server." })
        .option("captureTimeout", { number: true, default: 5000, description: "Timeout [msec] for capture a story." })
    ...
    const { outDir, flat, include, exclude, viewport, parallel, silent, verbose, serverTimeout, serverCmd, captureTimeout, captureMaxRetryCount, metricsWatchRetryCount, viewportDelay, reloadAfterChangeViewport, disableCssAnimation, } = setting.argv;
    const opt = {
        storybookUrl,
        outDir,
        flat,
        ...
        logger: new logger_1.Logger(verbose ? "verbose" : silent ? "silent" : "normal"),
    };
    return opt;
}
const start = Date.now();
const opt = createOptions();
const { logger } = opt, rest = __rest(opt, ["logger"]);
